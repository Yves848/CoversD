//********************************************************************************************************************************
//*                                                                                                                              *
//*     APEv2 Library 1.0.13.19 © 3delite 2012-2019                                                                              *
//*     See APEv2 Library Readme.txt for details.                                                                                *
//*                                                                                                                              *
//* Licenses available for usage of this component:                                                                              *
//* Freeware License: €15                                                                                                        *
//* 	http://www.shareit.com/product.html?productid=300953174                                                                  *
...
type
    ...
    public
        ...
        Constructor Create(_Parent: TAPEv2Tag);
        Destructor Destroy; override;
        function GetAsText: String;
        function GetAsTextANSI: String;
    ...
    public
        ...
        function AddBinaryFrame(Name: String; BinaryStream: TStream; DataSize: Integer): Integer;
...
Constructor TAPEv2Frame.Create(_Parent: TAPEv2Tag);
begin
    Inherited Create;
    Self.Parent := _Parent;
    Name := '';
...
function TAPEv2Frame.GetAsTextANSI: String;
var
    i: Integer;
    Data: Byte;
    Bytes: TBytes;
...
function TAPEv2Frame.GetIndex: Integer;
begin
    Result := Parent.Frames.IndexOf(Self);
end;
    ...
    while Stream.Position < Stream.Size do begin
        ...
        try
            ...
            if NameStr = '' then begin
                NameStr := TEncoding.ANSI.GetString(Bytes);
            end;
        except
            try
                NameStr := TEncoding.ANSI.GetString(Bytes);
        ...
        try
            ...
            if ValueStr = '' then begin
                ValueStr := TEncoding.ANSI.GetString(Bytes);
            end;
        except
            try
                ValueStr := TEncoding.ANSI.GetString(Bytes);
        ...
        List.Append(TrimRight(NameStr) + '=' + TrimRight(ValueStr));
        Result := True;
    end;
    Stream.Seek(0, soBeginning);
    AlreadyParsed := True;
...
begin
    ...
    if SameText(Name, 'Cover Art (Front)') then begin
        Result := True;
        Exit;
    end;
    try
        try
            ...
            while Offset < DataSize do begin
                if Length(Bytes) = 0 then begin
                    ByteCounter := 0;
                    while (PPicture^ <> 0)
                    AND (Offset < DataSize)
                ...
                //* JPEG
                if PPicture^ = $FF then begin
                    Inc(PPicture);
                    Inc(Offset);
                    if PPicture^ = $D8 then begin
                ...
                //* PNG 89 50 4E 47
                if PPicture^ = $89 then begin
                    Inc(PPicture);
                    Inc(Offset);
                    if PPicture^ = $50 then begin
                        Inc(PPicture);
                        Inc(Offset);
                        if PPicture^ = $4E then begin
                            Inc(PPicture);
                            Inc(Offset);
                            if PPicture^ = $47 then begin
                                Result := True;
                                Break;
                            end;
                        end;
                ...
                if PPicture^ = $47 then begin
                    Inc(PPicture);
                    Inc(Offset);
                    if PPicture^ = $49 then begin
                        Inc(PPicture);
                        Inc(Offset);
                        if PPicture^ = $46 then begin
                            Inc(PPicture);
                            Inc(Offset);
                            if PPicture^ = $38 then begin
                                //PictureFormat := pfGIF;
                                //Dec(PPicture, 3);
                                //Dec(Offset, 3);
                                Result := True;
                ...
                //* BMP 42 4D
                if PPicture^ = $42 then begin
                    Inc(PPicture);
                    Inc(Offset);
                    if PPicture^ = $4D then begin
                        //PictureFormat := pfBMP;
                        //Dec(PPicture);
                        //Dec(Offset);
                        Result := True;
...
begin
    Inherited;
    Frames := TList<TAPEv2Frame>.Create;
    Clear;
...
function TAPEv2Tag.GetFrameIndexOfCoverArtIndex(CoverArtIndex: Integer): Integer;
var
    i: Integer;
    Counter: Integer;
begin
    ...
    try
        ...
        try
            ...
            //if NOT APEv2ValidTag(TagStream) then begin
                ...
                if (ReadID3v1ID[0] = ID3v1ID[0])
                AND (ReadID3v1ID[1] = ID3v1ID[1])
                AND (ReadID3v1ID[2] = ID3v1ID[2])
                then begin
            ...
            //end;
            if TagStream.Read(APEv2Header, SizeOf(TAPEv2Header)) <> SizeOf(TAPEv2Header) then begin
            ...
            if (APEv2Header.Version <> 2000) then begin
                if (APEv2Header.Version <> 1000) then begin
                    Result := APEV2LIBRARY_ERROR_NOT_SUPPORTED_VERSION;
                    Exit;
                end;
            end;
            {
            if APEv2Header.Flags AND NOT $80000001 <> 0 then begin
                Result := APEV2LIBRARY_ERROR_CORRUPT;
                Exit;
            end;
            }
            TagStream.Seek(- APEv2Header.TagSize, soCurrent);
            for i := 0 to APEv2Header.ItemCount - 1 do begin
                FrameName := '';
                TagStream.Read(DataSize, 4);
                TagStream.Read(DataFlags, 4);
                if DataFlags AND NOT $7 <> 0 then begin
                    Result := APEV2LIBRARY_ERROR_CORRUPT;
                    //Exit;
                end;
                ...
                case (DataFlags and $6) SHR 1 of
                    0: begin
                        with AddFrame(FrameName) do begin
        ...
        //* Convert APEv1 ANSI text tags to APEv2 UTF8 format
        if APEv2Header.Version = 1000 then begin
            for i := 0 to Frames.Count - 1 do begin
                if Frames[i].Format = ffText then begin
                    Frames[i].SetAsText(Frames[i].GetAsTextANSI);
                end;
            end;
        end;
...
function TAPEv2Tag.LoadFromFile(_FileName: String): Integer;
var
...
begin
    ...
    try
        ...
        if (Result = APEV2LIBRARY_SUCCESS)
        OR (Result = APEV2LIBRARY_ERROR_NOT_SUPPORTED_VERSION)
        then begin
...
begin
    ...
    try
        ...
        Result := TAPEv2Frame.Create(Self);
        Result.Name := Name;
        Frames.Add(Result);
    except
        //*
...
function TAPEv2Tag.SaveToFile(_FileName: String): Integer;
var
    TagStream: TStream;
begin
    //Result := APEV2LIBRARY_ERROR;
    ...
    try
        try
            ...
            if NOT FileExists(_FileName) then begin
                ...
                TagStream := TFileStream.Create(_FileName, fmOpenReadWrite OR fmShareDenyWrite);
            end;
...
begin
    ...
    try
        try
            ...
            if APEv2ValidTag(Stream) then begin
                APEv2TagExists := True;
            end;
            if NOT APEv2TagExists then begin
                if ID3v1Stream.Size <> 0 then begin
                    Stream.Seek(- 128, soEnd);
                end else begin
                    Stream.Seek(0, soEnd);
                end;
                ...
                if Stream.Read(APEv2Header, SizeOf(TAPEv2Header)) <> SizeOf(TAPEv2Header) then begin
                    Result := APEV2LIBRARY_ERROR_NOT_SUPPORTED_FORMAT;
                    Exit;
                end;
                if APEv2Header.Reserved <> 0 then begin
            ...
            APEv2Header.ItemCount := Frames.Count;
            ...
            if Assigned(ID3v1Stream) then begin
...
function TAPEv2Tag.AddBinaryFrame(Name: String; BinaryStream: TStream; DataSize: Integer): Integer;
var
    PreviousPosition: Int64;
begin
    with AddFrame(Name) do begin
...
procedure TAPEv2Tag.ReSetAlreadyParsedState;
var
    i: Integer;
begin
    for i := 0 to Frames.Count - 1 do begin
...
begin
    ...
    try
        try
            ...
            ID3v1Stream := TMemoryStream.Create;
            if TagStream.Size >= 128 then begin
                TagStream.Seek(- 128, soEnd);
                TagStream.Read(ReadID3v1ID, 3);
            end;
            if (ReadID3v1ID[0] = ID3v1ID[0])
...
begin
    ...
    try
        try
            ...
            if NOT APEv2ValidTag(Stream) then begin
                Result := APEV2LIBRARY_ERROR_NO_TAG_FOUND;
                Exit;
            end;
            if ID3v1Stream.Size <> 0 then begin
...
begin
    ...
    case ErrorCode of
        APEV2LIBRARY_SUCCESS: Result := 'Success.';
        APEV2LIBRARY_ERROR: Result := 'Unknown error occured.';
        APEV2LIBRARY_ERROR_NO_TAG_FOUND: Result := 'No APE(v2) tag found.';
        APEV2LIBRARY_ERROR_EMPTY_TAG: Result := 'APE(v2) tag is empty.';
        APEV2LIBRARY_ERROR_EMPTY_FRAMES: Result := 'APE(v2) tag contains only empty frames.';
        APEV2LIBRARY_ERROR_OPENING_FILE: Result := 'Error opening file.';
        APEV2LIBRARY_ERROR_READING_FILE: Result := 'Error reading file.';
        APEV2LIBRARY_ERROR_WRITING_FILE: Result := 'Error writing file.';
        APEV2LIBRARY_ERROR_CORRUPT: Result := 'Error: corrupt file.';
        //APEV2LIBRARY_ERROR_DOESNT_FIT: Result := 'Error: APE(v2) tag doesn''t fit into the file.';
        APEV2LIBRARY_ERROR_NOT_SUPPORTED_VERSION: Result := 'Error: not supported APE tag version.';
        APEV2LIBRARY_ERROR_NOT_SUPPORTED_FORMAT: Result := 'Error: not supported file format.';
