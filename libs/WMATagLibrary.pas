//********************************************************************************************************************************
//*                                                                                                                              *
//*     WMA Tag Library 1.0.17.24 © 3delite 2013-2020                                                                            *
//*     See WMA Tag Library ReadMe.txt for details.                                                                              *
//*                                                                                                                              *
//* Licenses available for usage of this component:                                                                              *
//* Freeware License: €15                                                                                                        *
//*     http://www.shareit.com/product.html?productid=300953216                                                                  *
...
type
    IWMHeaderInfo2 = interface(IWMHeaderInfo)
        ...
        function GetCodecInfo(wIndex: LongWord; var pcchName: Word; pwszName: PWideChar; var pcchDescription: Word;
          pwszDescription: PWideChar; out pCodecType: WMT_CODEC_INFO_TYPE; var pcbCodecInfo: Word; pbCodecInfo: PBYTE): HRESULT; stdcall;
    end;
...
type
    ...
    public
        ...
        function SetAsInteger(Value: Int64; _Format: TWMTAttrDataType): Boolean;
    ...
    public
        FileName: String;
        Loaded: Boolean;
        Frames: TList<TWMATagFrame>;
        NormalizeRatingTo255: Boolean;
        Constructor Create;
        Destructor Destroy; override;
        ...
        function AddFrame(const Name: String): TWMATagFrame;
        ...
        function ReadAsInteger(const Name: String): Int64;
        procedure SetTextFrameText(const Name: String; const Text: String);
        procedure SetListFrameText(const Name: String; List: TStrings);
        function SetAsInteger(const Name: String; Value: Int64; Format: TWMTAttrDataType): Boolean;
        procedure Clear;
        ...
        function GetAttribIndex(const Attrib: String): Integer;
        function LoadTags(_FileName: String): Integer;
        ...
        function GetReportedFileSize: Int64;
        ...
        property CodecInfo: String read FCodecInfo;
        property CodecDescription: String read FCodecDescription;
    end;

    PCardinal = ^Cardinal;

    function RemoveWMATagFromFile(FileName: String): Integer;
...
var
    WMATagLibraryNormalizeRatingTo255: Boolean = False;
...
type
    T_WMCreateEditor = function(out ppEditor: IWMMetadataEditor): HRESULT; stdcall;

var
    DllHandleWMVCORE: THandle;
    _WMCreateEditor: T_WMCreateEditor;
...
begin
    Result := '';
    if Format = WMT_TYPE_STRING then begin
        ...
        Stream.Seek(0, soBeginning);
        Result := Str;
    end else if Format = WMT_TYPE_GUID then begin
        if GetAsGUID(GUID) then begin
...
function TWMATagFrame.GetIndex: Integer;
begin
...
function TWMATagFrame.SetAsText(const Text: String): Boolean;
var
    OLEString: PWideChar;
begin
    try
...
function TWMATagFrame.SetAsGUID(Value: TGUID): Boolean;
begin
    Stream.Clear;
    Stream.Write(Pointer(@Value.D1)^, 16);
    Stream.Seek(0, soBeginning);
    Format := WMT_TYPE_GUID;
    Result := True;
end;

function TWMATagFrame.SetAsInteger(Value: Int64; _Format: TWMTAttrDataType): Boolean;
var
    ValueBool: DWord;
    ValueWord: Word;
    ValueDWord: DWord;
...
begin
    try
        ...
        for i := 0 to List.Count - 1 do begin
            NameStr := List.Names[i];
            ValueStr := List.ValueFromIndex[i];
            {$IFDEF FPC}
            NameBytes := TEncoding.Unicode.GetBytes(NameStr);
            Stream.Write(NameBytes[0], Length(NameBytes));
            {$ELSE}
            Stream.Write(Pointer(NameStr)^, Length(NameStr) * 2);
            {$ENDIF}
            Data := $0D;
            Stream.Write(Data, 1);
            Data := $0A;
            Stream.Write(Data, 1);
            {$IFDEF FPC}
            ValueBytes := TEncoding.Unicode.GetBytes(ValueStr);
            Stream.Write(ValueBytes[0], Length(ValueBytes));
            {$ELSE}
            Stream.Write(Pointer(ValueStr)^, Length(ValueStr) * 2);
            {$ENDIF}
            Data := $0D;
            Stream.Write(Data, 1);
            Data := $0A;
...
function TWMATagFrame.GetAsGUID(out Value: TGUID): Boolean;
begin
    Stream.Seek(0, soBeginning);
    Stream.Read(Pointer(@Value.D1)^, 16);
    Result := True;
end;
...
begin
    with AddFrame(g_wszWMPicture) do begin
        if SetCoverArtFrame(Index, PictureStream, MIMEType, PictureType, Description) then begin
            Result := Index;
        end else begin
            DeleteFrame(Index);
            Result := - 1;
...
function TWMATag.AddFrame(const Name: String): TWMATagFrame;
begin
    Result := nil;
    try
...
begin
    ...
    for i := 0 to Frames.Count - 1 do begin
        if Name = UpperCase(Frames[i].Name) then begin
            Result := i;
            Break;
        end;
    end;
...
function TWMATag.ReadFrameByNameAsList(const Name: String; var List: TStrings): Boolean;
var
    i: Integer;
    l: Integer;
begin
...
function TWMATag.ReadFrameByNameAsText(const Name: String): String;
var
    i: Integer;
    l: Integer;
begin
...
procedure TWMATag.ReSetAlreadyParsedState;
var
    i: Integer;
begin
    for i := 0 to Frames.Count - 1 do begin
...
function TWMATag.ReadAsInteger(const Name: String): Int64;
var
    i: Integer;
    l: Integer;
begin
...
begin
    ...
    if (Index >= Frames.Count)
    OR (Index < 0)
    OR (Frames[Index].Format <> WMT_TYPE_BINARY)
    then begin
        Exit;
    end;
    try
...
begin
    ...
    try
        ...
        Frames[Index].Stream.Seek(SizeOf(Pointer), soFromCurrent);
        //* Read MIMEType
        MIMEType := Pointer(NativeUInt(Frames[Index].Stream.Memory) + UInt64(Frames[Index].Stream.Position));
        repeat
            Frames[Index].Stream.Read(DataWord, 2);
            if DataWord <> 0 then begin
        ...
        until DataWord = 0;
        //* Read description
        Description := Pointer(NativeUInt(Frames[Index].Stream.Memory) + UInt64(Frames[Index].Stream.Position));
        repeat
            Frames[Index].Stream.Read(DataWord, 2);
            if DataWord <> 0 then begin
        ...
        until DataWord = 0;
        Data := Pointer(NativeUInt(Frames[Index].Stream.Memory) + UInt64(Frames[Index].Stream.Position));
        Frames[Index].Stream.Seek(0, soBeginning);
        Result := True;
    except
        Result := False;
...
begin
    Result := ReadAsInteger(g_wszWMDuration) div 10000;
end;

function TWMATag.GetReportedFileSize: Int64;
begin
...
begin
    Result := 0;
    for i := 0 to Frames.Count - 1 do begin
        Result := Result + (Length(Frames[i].Name) + 1) * 2 + Integer(Frames[i].Stream.Size) + 4 + 1; //* Just guessing
    end;
end;
...
begin
    for i := Count - 1 downto 0 do begin
        if Frames[i].Name = g_wszWMPicture then begin
            DeleteFrame(i);
        end;
    end;
end;
...
function TWMATag.GetFrameIndexOfCoverArtIndex(CoverArtIndex: Integer): Integer;
var
    i: Integer;
    Counter: Integer;
begin
...
constructor TWMATag.Create;
begin
    Frames := TList<TWMATagFrame>.Create;
    Frames.Capacity := 256;
    NormalizeRatingTo255 := WMATagLibraryNormalizeRatingTo255;
end;
...
function TWMATag.GetAttribIndex(const Attrib: string): Integer;
var
...
function TWMATag.LoadTags(_FileName: String): Integer;
var
    AttributeCount: Word;
    ...
    pdwDataLength: DWORD;
    {$IFDEF FPC}
    FilesName: WideString;
    {$ENDIF}
    CodecInfo: PWideChar;
    CodecDescription: PWideChar;
    TmpWord: Word;
    ...
    RatingValue: Integer;
begin
    if DllHandleWMVCORE = 0 then begin
        DllHandleWMVCORE := SafeLoadLibrary(LibNameWMVCORE);
        if DllHandleWMVCORE = 0 then begin
            Result := WMATAGLIBRARY_ERROR_COULDNTLOADDLL;
            Exit;
        end;
    end;
    _WMCreateEditor := T_WMCreateEditor(GetProcAddress(DllHandleWMVCORE, 'WMCreateEditor'));
    Clear;
    if (NOT FileExists(_FileName))
    OR (NOT Assigned(_WMCreateEditor))
    then begin
    ...
    try
        ...
        {$IFDEF FPC}
        FilesName := _FileName;
        ppEditor.Open(PWideChar(FilesName));
        {$ELSE}
        ppEditor.Open(PWideChar(_FileName));
        {$ENDIF}
        HR := ppEditor.QueryInterface(IID_IWMHeaderInfo3, ppHeaderInfo3);
        if Failed(HR)
        OR (NOT Assigned(ppHeaderInfo3))
        then begin
        ...
        if AttributeCount > 0 then begin
            Loaded := True;
        end;
        for I := 0 to AttributeCount - 1 do begin
            ...
            with AddFrame(pwszName) do begin
                Format := pType;
                ...
                if NormalizeRatingTo255 then begin
                    //if (pwszName = g_wszWMRating)
                    if (pwszName = g_wszWMSharedUserRating)
                    then begin
                        RatingValue := GetAsInteger;
                        RatingValue := Trunc((RatingValue / 99) * High(Byte));
                        SetAsInteger(RatingValue, Format);
                    end;
                end;
            end;
            FreeMem(pwszName);
        ...
        //* Codec info
        CodecInfo := AllocMem(256);
        CodecDescription := AllocMem(256);
        try
            ppHeaderInfo3.GetCodecInfo(0, TmpWord, CodecInfo, TmpWord, CodecDescription, pCodecType, pcbCodecInfo, @pbCodecInfo);
            FCodecInfo := CodecInfo;
            FCodecDescription := CodecDescription;
        finally
            FreeMem(CodecInfo);
            FreeMem(CodecDescription);
        end;
        Result := WMATAGLIBRARY_SUCCESS;
        ppHeaderInfo3 := nil;
    finally
        if Assigned(ppEditor) then begin
            ppEditor.Close;
        end;
        ppEditor := nil;
        if DllHandleWMVCORE <> 0 then begin
...
function TWMATag.SaveToFile(_FileName: String): Integer;
var
...
begin
    ...
    try
        ...
        for i := DeleteAttributeCount - 1 downto 0 do begin
            wIndex := Word(I);
        ...
        for i := 0 to Frames.Count - 1 do begin
            if Frames[i].Name = 'WM/Picture' then begin
                Continue;
            end;
            //* Rating
            if NormalizeRatingTo255 then begin
                //if (Frames[i].Name = g_wszWMRating)
                if (Frames[i].Name = g_wszWMSharedUserRating)
                then begin
                    RatingValue := Frames[i].GetAsInteger;
                    RatingValue := Trunc((RatingValue / High(Byte)) * 99);
                    Frames[i].SetAsInteger(RatingValue, Frames[i].Format);
                end;
            end;
            AttribName := PWideChar(Frames[i].Name);
...
function TWMATag.SetAsInteger(const Name: String; Value: Int64; Format: TWMTAttrDataType): Boolean;
var
    i: Integer;
    l: Integer;
begin
...
procedure TWMATag.SetListFrameText(const Name: String; List: TStrings);
var
    i: Integer;
    l: Integer;
begin
...
begin
    ...
    try
        ...
        Inc(DataPointer, (Length(MIMEType) + 1) * 2);
        Frames[Index].Stream.Write(DataPointer, SizeOf(DataPointer));
        //* Update data pointer
        ...
        Inc(DataPointer, (Length(Description) + 1) * 2);
        Frames[Index].Stream.Write(DataPointer, SizeOf(DataPointer));
        Frames[Index].Stream.Seek(0, soBeginning);
        Result := True;
    except
...
procedure TWMATag.SetTextFrameText(const Name, Text: String);
var
    i: Integer;
    l: Integer;
begin
...
begin
    ...
    try
        if GetCoverArtFromFrame(Index, PictureStream, MIMEType, PictureType, Description) then begin
...
function TWMATag.LoadFromFile(_FileName: String): Integer;
var
    fFileName: PWideChar;
begin
    try
        ...
        FFileSize := NGetFileSize(_FileName);
        if NOT FileExists(_FileName) then begin
            Result := WMATAGLIBRARY_ERROR_FILENOTFOUND;
            Exit;
        end;
        fFileName := StringToOleStr(_FileName);
...
begin
    ...
    case ErrorCode of
        WMATAGLIBRARY_SUCCESS: Result := 'Success.';
        WMATAGLIBRARY_ERROR: Result := 'Unknown error occured.';
        WMATAGLIBRARY_ERROR_FILENOTFOUND: Result := 'Error opening file.';
        WMATAGLIBRARY_ERROR_COULDNTLOADDLL: Result := 'Error loading ' + LibNameWMVCORE + '.';
        WMATAGLIBRARY_ERROR_COULDNOTCREATEMETADATAEDITOR: Result := 'Error: could not create WMA meta data editor.';
        WMATAGLIBRARY_ERROR_COULDNOTQIFORIWMHEADERINFO3: Result := 'Error: could not query for WMA IWMHeaderInfo3.';
        WMATAGLIBRARY_ERROR_COULDNOTQUERY_ATTRIBUTE_COUNT: Result := 'Error: could not query WMA attribute count.';
    end;
end;
