...
{                                                                             }
{ Version 2.1 (February 2015) by 3delite                                      }
{   - Added support for the NextGen compiler                                  }
{   - Added LoadFromStream() function                                         }
{   - Added SV8 support                                                       }
{   - Removed tag format dependencies                                         }
{                                                                             }
...
type
  ...
  TMPEGplus = class(TObject)
    ...
    public
      ...
      function LoadFromFile(const FileName: String; ID3v2Size, RestTagsTotalSize: Integer): Boolean;{ Load header }
      function LoadFromStream(Stream: TStream; ID3v2Size, RestTagsTotalSize: Integer): Boolean;{ Load header }
      ...
      property SV8: Boolean read FSV8;
  end;
...
function ReadHeader(SourceFile: TStream; var Header: HeaderRecord): Boolean;
var
    Transferred: Integer;
begin
  try
    Result := true;
    SourceFile.Seek(Header.ID3v2Size, soBeginning);
...
function GetEncoder(const Header: HeaderRecord): string; var   EncoderID : integer;
begin
...
begin
    ...
    if NOT FSV8 then begin
        ...
        CompressedSize := FFileSize - FTagsSize;
        //if FID3v1.Exists then Dec(FFileSize, 128);
        if (Result = 0) and (FFrameCount > 0) then begin
            Result := Round(CompressedSize * 8 * (FSampleRate/1000) / FFRameCount / 1152);
        end;
    end else begin
        CompressedSize := FFileSize - FTagsSize;
        Result := Round(CompressedSize / (125 * FGetDuration) + 0.5); // bitrate (Kbps) ;
    end;
end;
...
function TMPEGplus.LoadFromFile(const FileName: String; ID3v2Size, RestTagsTotalSize: Integer): Boolean;
var
    Stream: TFileStream;
begin
    ...
    try
        Result := LoadFromStream(Stream, ID3v2Size, RestTagsTotalSize);
    finally
        FreeAndNil(Stream);
    end;
end;

function TMPEGplus.LoadFromStream(Stream: TStream; ID3v2Size, RestTagsTotalSize: Integer): Boolean;
var
    Header: HeaderRecord;
...
begin
    { Reset data and load header from file to variable }
    FResetData;
    Header.ID3v2Size := ID3v2Size;
    FTagsSize := ID3v2Size + RestTagsTotalSize;
    //* Check if SV8 format
    ...
    then begin
        ...
        until (DataByte AND $80) = 0;
        //* Skrip CRC
        Stream.Seek(4, soCurrent);
        //* Stream version
        Stream.Read(DataByte, 1);
        FStreamVersion := DataByte;
        //* Sample count
        FSampleCount := 0;
        repeat
            Stream.Read(DataByte, 1);
            FSampleCount := FSampleCount SHL 7;
            Temp := DataByte SHL 1;
            Temp := Temp SHR 1;
            FSampleCount := FSampleCount OR Temp;
        until (DataByte AND $80) = 0;
        //* Beginning silence
        repeat
            Stream.Read(DataByte, 1);
        ...
        //* Channels
        Stream.Read(DataByte, 1);
        FChannelCount := (DataByte SHR 4) + 1;
        ...
        if (Result) and (Header.FileSize > 0) and (GetStreamVersion(Header) > 0) then begin
            FValid := True;
