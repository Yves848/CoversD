//********************************************************************************************************************************
//*                                                                                                                              *
//*     Flac Tag Library 2.0.24.55 © 3delite 2013-2019                                                                           *
...
//* If you are using the cover art managing functions (read and/or write) and/or Ogg Flac functions you can use it for free      *
//* programs/projects (non-money making use) with a freeware license but for shareware or commerical programs (or money making)  *
//* you need one of the following licenses:                                                                                      *
//* Freeware License: €15                                                                                                        *
//* 	http://www.shareit.com/product.html?productid=300953175                                                                  *
...
type
    ...
    public
        ...
        function CalculateTotalFrameSize: Int64;
    ...
    private
        ...
        function GetInfo(Stream: TStream; Buffered, SetTags: Boolean): Integer;
    ...
    public
        ...
        constructor Create;
        destructor Destroy; override;
        function LoadFromFile(const _FileName: String; Buffered: Boolean = True): Integer;
        function LoadFromStream(Stream: TStream; Buffered: Boolean = True): Integer;
        function SaveToFile(const _FileName: String; Buffered: Boolean = True): Integer;
        ...
        function AddTag(const Name: String): TVorbisComment;
        function DeleteFrame(FrameIndex: Integer): Boolean;
        procedure DeleteAllFrames;
        procedure DeleteAllCoverArts;
        function FrameExists(const Name: String): Integer;
        function AddTextTag(const Name: String; const Text: String): Integer;
        ...
        function GetCoverArtInfo(Index: Integer; var FlacTagCoverArtInfo: TFlacTagCoverArtInfo; ABytes: PBytes = nil): Boolean;
        ...
        property FileSize: Int64 read FFileSize;                    // File length (bytes)
        ...
        property AudioOffset: Int64 read FAudioOffset;            //offset of audio data
        property AudioPosition: Int64 read FAudioOffset;
        property AudioSize: Int64 read FAudioSize;
        property VCOffset: Int64 read FVCOffset;
    end;

    function RemoveFlacTagFromFile(const FileName: String; CreatePaddingSize: Integer = 0): Integer;
    function RemoveFlacTagFromStream(Stream: TStream; CreatePaddingSize: Integer = 0): Integer;
...
begin
    ...
    Source.Read2(Header, SizeOf(ID3v2Header));
...
begin
    ...
    for Index := 1 to Header.Segments do begin
        if Header.LacingValues[Index] > 0 then begin
            Destination.Read2(Data, Header.LacingValues[Index]);
...
begin
    ...
    try
        if Assigned(SourceStream) then begin
            ...
            FStream.Read2(FirstOGGHeader, SizeOf(TOggHeader));
...
begin
    ...
    try
        ...
        repeat
            FStream.Read2(Header, SizeOf(TOggHeader) - SizeOf(Header.LacingValues));
            FillChar(Header.LacingValues, SizeOf(Header.LacingValues), 0);
            FStream.Read2(Header.LacingValues, Header.Segments);
            ...
            if PageCounter = PageNumber then begin
                FStream.Seek(- (Int64(SizeOf(TOggHeader) - Int64(SizeOf(Header.LacingValues)))) - Int64(Header.Segments), soCurrent);
...
begin
    try
        ...
        OGGPage := TMemoryStream.Create;
        try
            while (Destination.Position < Destination.Size)
            ...
            do begin
                ...
                GetNextPage(OGGPage);
                OGGPage.Seek(0, soBeginning);
                OGGPage.Read2(Header, SizeOf(TOggHeader) - SizeOf(Header.LacingValues));
                FillChar(Header.LacingValues, SizeOf(Header.LacingValues), 0);
                OGGPage.Read2(Header.LacingValues, Header.Segments);
                OGGPage.Seek(0, soBeginning);
                ...
                Inc(PageCounter);
                if EndingPageNumber <> - 1 then begin
                    if PageCounter > EndingPageNumber then begin
                        Break;
                    end;
                end;
            end;
...
function TVorbisComment.GetIndex: Integer;
begin
    Result := Self.Parent.Tags.IndexOf(Self);
end;

function TVorbisComment.SetAsText(const Text: String): Boolean;
var
    Bytes: TBytes;
begin
    if Text <> '' then begin
...
begin
    ...
    while Stream.Position < Stream.Size do begin
        ...
        NameStr := TEncoding.UTF8.GetString(Bytes);
        SetLength(Bytes, 0);
        ByteCounter := 0;
        repeat
            Stream.Read2(Data, 1);
            ...
            Inc(ByteCounter);
        until Stream.Position >= Stream.Size;
        ValueStr := TEncoding.UTF8.GetString(Bytes);
...
function TVorbisComment.CalculateTotalFrameSize: Int64;
begin
    Result := Int64(Length(Name)) + 1 + Stream.Size;
end;

function TVorbisComment.Assign(VorbisComment: TVorbisComment): Boolean;
begin
    Clear;
    if VorbisComment <> nil then begin
...
begin
    if HeaderInfo then begin
        ...
        for i := 0 to Length( aMetaBlockOther ) - 1 do begin
            if Assigned(aMetaBlockOther[ i ].Data) then begin
...
begin
    Result := False;
    if StreamType = fstNativeFlac then begin
        Result := ((FHeader.StreamMarker[1] = Ord('f')) AND (FHeader.StreamMarker[2] = Ord('L')) AND (FHeader.StreamMarker[3] = Ord('a')) AND (FHeader.StreamMarker[4] = Ord('C')))
            AND (FChannels > 0)
            AND (FSampleRate > 0)
            AND (FBitsPerSample > 0)
            AND (FSamples > 0);
    end;
    if StreamType = fstOggFlac then begin
        Result := ((FOggFlacHeader.StreamInfo.StreamMarker[1] = Ord('f')) AND (FOggFlacHeader.StreamInfo.StreamMarker[2] = Ord('L')) AND (FOggFlacHeader.StreamInfo.StreamMarker[3] = Ord('a')) AND (FOggFlacHeader.StreamInfo.StreamMarker[4] = Ord('C')))
            AND (FChannels > 0)
            AND (FSampleRate > 0)
            AND (FBitsPerSample > 0)
            AND (FSamples > 0);
...
function TFlacTag.FrameExists(const Name: String): Integer;
var
    i: Integer;
begin
    Result := -1;
    for i := 0 to Tags.Count - 1 do begin
        if Name = Tags[i].Name then begin
            Result := i;
            Break;
        end;
    end;
...
begin
    if FIsValid then begin
        Result := FAudioSize / (FSamples * FChannels * FBitsPerSample / 8) * 100
    end else begin
        Result := 0;
    end;
end;
...
begin
    inherited;
    Tags := TList<TVorbisComment>.Create;
...
var
    i: Integer;
begin
    for i := CoverArtCount - 1 downto 0 do begin
        DeleteCoverArt(i);
    end;
end;
...
begin
    Result := False;
    if (FrameIndex >= Tags.Count)
    OR (FrameIndex < 0)
    then begin
        Exit;
    end;
    Tags[FrameIndex].Free;
    Tags.Delete(FrameIndex);
    Result := True;
end;
...
function TFlacTag.ReadFrameByNameAsList(const Name: String; var List: TStrings): Boolean;
var
    i: Integer;
    l: Integer;
begin
    ...
    i := 0;
    while (i <> l)
    AND (UpperCase(Tags[i].Name) <> UpperCase(Name))
    do begin
        inc(i);
    end;
    if i = l then begin
        ...
        if Tags[i].Format = vcfText then begin
            Result := Tags[i].GetAsList(List);
        end;
    end;
end;

function TFlacTag.ReadFrameByNameAsText(const Name: String): String;
var
    i: Integer;
    l: Integer;
begin
...
function TFlacTag.LoadFromFile(const _FileName: String; Buffered: Boolean = True): Integer;
var
...
begin
    ...
    try
        Result := GetInfo(FileStream, Buffered, True);
    finally
        FreeAndNil(FileStream);
    end;
end;

function TFlacTag.LoadFromStream(Stream: TStream; Buffered: Boolean = True): Integer;
begin
    Clear;
    try
        Result := GetInfo(Stream, Buffered, True);
    except
...
begin
    ...
    with FlacTagCoverArtInfo do begin
        ...
        Stream.Read2(Width, 4);
        Width := ReverseBytes(Width);
        Stream.Read2(Height, 4);
        Height := ReverseBytes(Height);
        Stream.Read2(ColorDepth, 4);
        ColorDepth := ReverseBytes(ColorDepth);
        Stream.Read2(NoOfColors, 4);
        NoOfColors := ReverseBytes(NoOfColors);
        Stream.Read2(LengthOfPictureData, 4);
...
function TFlacTag.GetCoverArtInfo(Index: Integer; var FlacTagCoverArtInfo: TFlacTagCoverArtInfo; ABytes: PBytes = nil): Boolean;
var
    i: Integer;
    Stream: TStream;
    MIMETypeLength: Cardinal;
...
begin
    ...
    with FlacTagCoverArtInfo do begin
        ...
        Stream.Read2(NoOfColors, 4);
        NoOfColors := ReverseBytes(NoOfColors);
        Stream.Read2(SizeOfPictureData, 4);
        SizeOfPictureData := ReverseBytes(SizeOfPictureData);
        if (ABytes <> nil) and (SizeOfPictureData > 0) then begin
            SetLength(ABytes^, SizeOfPictureData);
            Stream.Read2(ABytes^[0], SizeOfPictureData);
        end;
    end;
    Result := True;
end;
...
begin
    ...
    with MetaBlocksCoverArts[Index] do begin
...
begin
    Result := False;
    if (Index >= Length(MetaBlocksCoverArts))
    OR (Index < 0)
    then begin
        Exit;
    end;
    if Assigned(MetaBlocksCoverArts[Index].Data) then begin
        FreeAndNil(MetaBlocksCoverArts[Index].Data);
    end;
    i := 0;
    j := 0;
    while i <= Length(MetaBlocksCoverArts) - 1 do begin
        if MetaBlocksCoverArts[i].Data <> nil then begin
...
function TFlacTag.GetInfo(Stream: TStream; Buffered, SetTags: Boolean): Integer;
var
...
begin
    ...
    try
        ...
        try
            ...
            ID3v2Size := GetID3v2Size(LStream);
            LStream.Seek(ID3v2Size, soFromBeginning);
            // Read header data
            LStream.Read2(FHeader, SizeOf(FHeader));
            // Process data if loaded and header valid
            if ((FHeader.StreamMarker[1] = Ord('f')) AND (FHeader.StreamMarker[2] = Ord('L')) AND (FHeader.StreamMarker[3] = Ord('a')) AND (FHeader.StreamMarker[4] = Ord('C'))) then begin
                ...
                repeat // read more metadata blocks if available
                    ...
                    if iMetaType = META_VORBIS_COMMENT then begin  // read vorbis block
                        FVCOffset := LStream.Position;
                        FTagSize := iBlockLength;
                        FVorbisIndex := iIndex;
                        ReadTag(LStream, SetTags); // set up fields
                        ...
                        bPaddingFound := true;
                        LStream.Seek(FPadding, soCurrent); // advance into file till next block or audio data start
                    end else begin // all other
                        if iMetaType <= 6 then begin // is it a valid metablock ?
                            if (iMetaType = META_PADDING) then begin // set flag for fragmented padding blocks
                                FPaddingFragments := True;
                            end;
                            if iMetaType = META_COVER_ART then begin
                                if SetTags then begin
                                    CoverArtIndex := AddMetaDataCoverArt({aMetaDataBlockHeader,} LStream, iBlocklength {, iIndex});
                                    GetCoverArtInfoPointer(CoverArtIndex, Self.MetaBlocksCoverArts[CoverArtIndex].CoverArtInfo);
                                end else begin
                                    LStream.Seek(iBlocklength, soCurrent);
                                end;
                            end else begin
                                AddMetaDataOther(aMetaDataBlockHeader, LStream, iBlocklength, iMetaType);
                            end;
                ...
                LStream.Seek(ID3v2Size, soFromBeginning);
                LStream.Read2(OggHeader, SizeOf(TOggHeader));
                if (OggHeader.ID[1] = Ord('O'))
                AND (OggHeader.ID[2] = Ord('g'))
                AND (OggHeader.ID[3] = Ord('g'))
                AND (OggHeader.ID[4] = Ord('S'))
                then begin
                    LStream.Seek(ID3v2Size + $1C, soFromBeginning);
                    LStream.Read2(FOggFlacHeader, SizeOf(TOggFlacHeader));
                    if (FOggFlacHeader.PacketType = $7F)
                    AND ((FOggFlacHeader.Signature[1] = Ord('F')) AND (FOggFlacHeader.Signature[2] = Ord('L')) AND (FOggFlacHeader.Signature[3] = Ord('A')) AND (FOggFlacHeader.Signature[4] = Ord('C')))
                    AND (FOggFlacHeader.MajorVersion = 1)
                    AND ((FOggFlacHeader.StreamInfo.StreamMarker[1] = Ord('f')) AND (FOggFlacHeader.StreamInfo.StreamMarker[2] = Ord('L')) AND (FOggFlacHeader.StreamInfo.StreamMarker[3] = Ord('a')) AND (FOggFlacHeader.StreamInfo.StreamMarker[4] = Ord('C')))
                    then begin
                        StreamType := fstOggFlac;
                        with FOggFlacHeader.StreamInfo do begin
                            FChannels := (Info[13] shr 1 and $7 + 1);
                            FSampleRate := (Info[11] shl 12 or Info[12] shl 4 or Info[13] shr 4);
                            FBitsPerSample := (Info[13] and 1 shl 4 or Info[14] shr 4 + 1);
                            FSamples := (Info[15] shl 24 or Info[16] shl 16 or Info[17] shl 8 or Info[18]);
                        end;
                        if (FOggFlacHeader.StreamInfo.MetaDataBlockHeader[1] and $80) <> 0 then begin
                            Result := FLACTAGLIBRARY_ERROR_NO_TAG_FOUND;
                            Exit; //no metadata blocks exist
                        end;
                        OGGStream := TOGGStream.Create(LStream);
                        ...
                        try
                            ...
                            repeat // read more metadata blocks if available
                                ...
                                while Data.Position + 4 > Data.Size do begin
                                    Data.Seek(0, soEnd);
                                    if NOT OGGStream.GetNextPageData(Data) then begin
                                        Break;
                                    end;
                                    Data.Seek(PreviousPosition, soBeginning);
                                end;
                                Data.Read2( aMetaDataBlockHeader, 4 );
                                Inc(iIndex); // metadatablock index
                                iBlockLength := (aMetaDataBlockHeader[2] shl 16 or aMetaDataBlockHeader[3] shl 8 or aMetaDataBlockHeader[4]); //decode length
                                if iBlockLength <= 0 then begin
                                    FMetaBlocksSize := FMetaBlocksSize + 4;
                                ...
                                if iMetaType = META_VORBIS_COMMENT then begin  // read vorbis block
                                    ...
                                    //* Query more data if needed
                                    PreviousPosition := Data.Position;
                                    while Data.Position + iBlockLength > Data.Size do begin
                                        Data.Seek(0, soEnd);
                                        if NOT OGGStream.GetNextPageData(Data) then begin
                                            Break;
                                        end;
                                    ...
                                    //* Query more data if needed
                                    PreviousPosition := Data.Position;
                                    while Data.Position + FPadding > Data.Size do begin
                                        Data.Seek(0, soEnd);
                                        if NOT OGGStream.GetNextPageData(Data) then begin
                                            Break;
                                        end;
                        ...
                        if FOggFlacHeader.MajorVersion <> 1 then begin
            ...
            if FIsValid then begin
                FAudioOffset := LStream.Position;  // we need that to rebuild the file if nedeed
                FAudioSize := FFileSize - FAudioOffset;
                if FGetPlayTime <> 0 then begin
                    FBitrate := Round(((FFileSize - FAudioOffset) / 1000) * 8 / FGetPlayTime); //time to calculate average bitrate
                end;
            end else begin
                if Result = FLACTAGLIBRARY_ERROR then begin
                    Result := FLACTAGLIBRARY_ERROR_NOT_SUPPORTED_FORMAT;
                end;
            end;
        end;
...
begin
    ...
    for i := 0 to Count - 1 do begin
        if SameText(Tags[i].Name, TagName) then begin
...
function TFlacTag.AddTag(const Name: String): TVorbisComment;
begin
    Result := nil;
    try
...
function TFlacTag.AddTextTag(const Name: String; const Text: String): Integer;
begin
    with AddTag(Name) do begin
        SetAsText(Text);
        Result := Index;
    end;
end;
...
begin
    ...
    Data := TMemoryStream.Create;
    try
        Source.Read2(DataSize, SizeOf(DataSize)); // vendor
        ...
        Source.Read2(TagCount, SizeOf(TagCount)); //field count
        FExists := (TagCount > 0);
        for i := 0 to TagCount - 1 do begin
            Source.Read2(DataSize, SizeOf(DataSize));
            if DataSize <= 0 then begin
                Continue;
            end;
            Data.Clear;
            ...
            //Data.Seek(0, soEnd);
            //Data.Write(ZeroByte, 1);
            if NOT SetTagFields then begin
            ...
            if SeparatorPos > 0 then begin
                ...
                with AddTag(FieldName) do begin
                    FieldValue := Copy(TempString, SeparatorPos + 1, Length(TempString));
                    Bytes := TEncoding.UTF8.GetBytes(FieldValue);
                    Stream.Write(Bytes[0], Length(Bytes));
                    Stream.Seek(0, soBeginning);
                    Format := vcfText;
                end;
...
var
    ...
    begin
        if sData <> '' then begin
            Bytes := TEncoding.UTF8.GetBytes(sID + '=' + sData);
            iTmp := Length(Bytes);
...
function TFlacTag.SaveToFile(const _FileName: String; Buffered: Boolean = True): Integer;
var
    VorbisBlock: TStream;
    FileStream: TStream;
begin
    ...
    try
        ...
        if GetInfo(FileStream, Buffered, False) <> FLACTAGLIBRARY_SUCCESS then begin
            Exit;
        end;
    finally
        {$IFDEF NEXTGEN}
    ...
    VorbisBlock := TMemoryStream.Create;
    try
        if NOT ConstructVorbisBlock(VorbisBlock) then begin
            Exit;
        end;
        if StreamType = fstNativeFlac then begin
            Result := RebuildFile(_FileName, VorbisBlock);
        end;
        if StreamType = fstOggFlac then begin
            Result := RebuildOggFile(_FileName, VorbisBlock);
        end;
...
function TFlacTag.SaveToStream(Stream: TStream; Buffered: Boolean = True): Integer;
var
    VorbisBlock: TStream;
begin
    Result := FLACTAGLIBRARY_ERROR;
    try
        Stream.Seek(0, soBeginning);
        //reload all except tag fields
        if GetInfo(Stream, Buffered, False) <> FLACTAGLIBRARY_SUCCESS then begin
            Exit;
        end;
    except
        Exit;
...
procedure TFlacTag.SetListFrameText(const Name: String; List: TStrings);
var
    i: Integer;
    l: Integer;
begin
...
procedure TFlacTag.SetTextFrameText(const Name: String; const Text: String);
var
    i: Integer;
    l: Integer;
begin
...
function TFlacTag.DeleteFrameByName(const Name: String): Boolean;
var
    i: Integer;
    l: Integer;
begin
    l := Tags.Count;
    i := 0;
    while (i <> l) and (UpperCase(Tags[i].Name) <> UpperCase(Name)) do begin
        inc(i);
    end;
    if i = l then begin
        Result := False;
...
begin
    ...
    try
        ...
        // set up file
        if (FPadding <= VorbisBlock.Size - FTagSize)
        OR (FMetaBlocksSize <> NewMetaBlocksSize)
        OR ForceReWrite
        then begin // no room rebuild the file from scratch
            bRebuild := True;
            //* Working with files
            if NOT Assigned(Stream) then begin
                BufferName := FileName + '~';
            ...
            Source.Read2( oldHeader, sizeof(oldHeader));
            ...
            if bRearange then begin
                ...
                Destination.Seek(FVCOffset - 4, soBeginning);
            end;
        end;
        ...
        if bRebuild then begin
            ...
            if (FTagSize + FMetaBlocksSize) > (VorbisBlock.Size + NewMetaBlocksSize) then begin // tag got smaller increase padding
        ...
        if (FPadding <> iNewPadding) or bRearange then begin // fill the block with zeros
            SetLength(PaddingBytes, iNewPadding);
            Destination.Write(PaddingBytes[0], Length(PaddingBytes));
        end;
        // finish
        if bRebuild then begin // time to put back the audio data...
            ...
            if NOT Assigned(Stream) then begin
                ...
                Source.DisposeOf;
                Source := nil;
                Destination.DisposeOf;
                Destination := nil;
                {$ELSE}
                FreeAndNil(Source);
                FreeAndNil(Destination);
                {$ENDIF}
                if (SysUtils.DeleteFile(FileName)) AND (RenameFile(BufferName, FileName)) then begin //Replace old file and delete temporary file
                    Result := FLACTAGLIBRARY_SUCCESS;
                end else begin
                    Result := FLACTAGLIBRARY_ERROR_WRITING_FILE;
                end;
        ...
        if Assigned(MetaBlocks) then begin
        ...
        if NOT Assigned(Stream) then begin
            ...
            if Destination <> Stream then begin
                Stream.Size := 0;
                Stream.Seek(0, soBeginning);
        ...
        // Access error
        if FileExists(BufferName) then begin
...
var
    ...
    begin
        ...
        try
            ...
            Data := 0;
            for k := 0 to PaddingSize - 1 do begin
    ...
    try
        ...
        try
            ...
            try
                ...
                for i := 0 to iMetaCount - 1 do begin
                    MetaBlock.Clear;
                    aMetaBlockOther[i].MetaDataBlockHeader[1] := (aMetaBlockOther[i].MetaDataBlockHeader[1] AND $7f); // not last
                    if aMetaBlockOther[i].BlockType = META_PADDING then begin
                        Continue;
                    end else begin
                        aMetaBlockOther[i].MetaDataBlockHeader[2] := Byte((aMetaBlockOther[i].Data.Size SHR 16 ) AND 255 );
                        aMetaBlockOther[i].MetaDataBlockHeader[3] := Byte((aMetaBlockOther[i].Data.Size SHR 8 ) AND 255 );
            ...
            if NewBlocksSize <> AvailableSpace then begin
                //* Add padding
                if (NewBlocksSize < AvailableSpace)
                OR (AvailableSpace - NewBlocksSize > PaddingSizeToWrite)
                then begin
                    iNewPadding := 4;
                    repeat
                        Inc(iNewPadding);
                        WrappedPaddingSize := OGGStream.CalculateWrappedStreamSize(iNewPadding);
                    until (WrappedBlocks.Size + WrappedPaddingSize >= AvailableSpace)
                    OR (iNewPadding > PaddingSizeToWrite);
                    if WrappedBlocks.Size + WrappedPaddingSize <> AvailableSpace then begin
                        Rebuild := True;
                    end else begin
            ...
            Destination.Seek(ID3v2Size + $1C, soBeginning);
            Destination.Read2(FOggFlacHeader, SizeOf(TOggFlacHeader));
            ...
            if Rebuild then begin
                ...
                if (OggPageHeader.PageNumber <> OggPageCount + 1)
                then begin
                    Destination.Seek(ID3v2Size + $4F + WrappedBlocks.Size, soBeginning);
                    NewOGGStream.ReNumberPages(OggPageCount + 1, - 1, Destination);
                end;
        ...
        if Rebuild then begin
            if NOT Assigned(Stream) then begin
                ...
                if NOT RenameFile(BufferName, FileName) then begin
                    Result := FLACTAGLIBRARY_ERROR_WRITING_FILE;
                end;
            end;
        end;
...
procedure TFlacTag.ReSetAlreadyParsedState;
var
    i: Integer;
begin
    for i := 0 to Tags.Count - 1 do begin
...
begin
    ...
    for i := 0 to FlacTag.Tags.Count - 1 do begin
        Self.AddTag('').Assign(FlacTag.Tags[i]);
    end;
    for i := 0 to Length(FlacTag.MetaBlocksCoverArts) - 1 do begin
...
begin
    ...
    try
        ...
        FlacTag.ForceReWrite := True;
        FlacTag.PaddingSizeToWrite := CreatePaddingSize;
...
begin
    ...
    case ErrorCode of
        FLACTAGLIBRARY_SUCCESS: Result := 'Success.';
        FLACTAGLIBRARY_ERROR: Result := 'Unknown error occured.';
        FLACTAGLIBRARY_ERROR_NO_TAG_FOUND: Result := 'No Flac tag found.';
        FLACTAGLIBRARY_ERROR_EMPTY_TAG: Result := 'Flac tag is empty.';
        FLACTAGLIBRARY_ERROR_EMPTY_FRAMES: Result := 'Flac tag contains only empty frames.';
        FLACTAGLIBRARY_ERROR_OPENING_FILE: Result := 'Error opening file.';
        FLACTAGLIBRARY_ERROR_READING_FILE: Result := 'Error reading file.';
        FLACTAGLIBRARY_ERROR_WRITING_FILE: Result := 'Error writing file.';
        FLACTAGLIBRARY_ERROR_NOT_SUPPORTED_VERSION: Result := 'Error: not supported Flac tag version.';
        FLACTAGLIBRARY_ERROR_NOT_SUPPORTED_FORMAT: Result := 'Error not supported file format.';
        FLACTAGLIBRARY_ERROR_NEED_EXCLUSIVE_ACCESS: Result := 'Error: file is locked. Need exclusive access to write Flac tag to this file.';
    end;
end;
