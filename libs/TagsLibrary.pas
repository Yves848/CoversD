//********************************************************************************************************************************
//*                                                                                                                              *
//*     Tags Library 1.0.89.155 © 3delite 2014-2020                                                                              *
//*     See Tags Library Readme.txt for details.                                                                                 *
//*                                                                                                                              *
//* Licenses available for usage of this component:                                                                              *
//* Freeware License: €50                                                                                                        *
//* 	http://www.shareit.com/product.html?productid=300953218                                                                  *
...
Const
    TAGSLIBRARY_VERSION = $010089155;
    TAGSLIBRARY_VERSION_STR = 'Tags Library 1.0.89.155';
...
Const
    ...
    BEXT_UMID = 'BEXT UMID';
    BEXT_LoudnessValue = 'BEXT LoudnessValue';
    BEXT_LoudnessRange = 'BEXT LoudnessRange';
    BEXT_MaxTruePeakLevel = 'BEXT MaxTruePeakLevel';
    BEXT_MaxMomentaryLoudness = 'BEXT MaxMomentaryLoudness';
    BEXT_MaxShortTermLoudness = 'BEXT MaxShortTermLoudness';
...
Const
    ...
    MAGIC_TIFF_LE   = $002A4949;    //* Little endian (little endian form)
    MAGIC_TIFF_BE   = $2A004D4D;    //* Big endian (little endian form)

Const
    DEFAULT_UPPERCASE_FIELD_NAMES = True;

type
    TTagType = (ttNone, ttAutomatic, ttAPEv2, ttFlac, ttID3v1, ttID3v2, ttMP4, ttOpusVorbis, ttWAV, ttWMA, ttMP4Xtra);
...
type
    TTagPictureFormat = (tpfUnknown, tpfJPEG, tpfPNG, tpfBMP, tpfGIF, tpfTIFF);
...
type
    ...
    public
        ...
        Constructor Create(_Parent: TTags);
    ...
    public
        ...
        function SaveToFile(_FileName: String; TagType: TTagType = ttAutomatic): Integer;
        function SaveToStream(Stream: TStream; Buffered: Boolean = True; TagType: TTagType = ttAutomatic): Integer;
        procedure LoadTags;
        function Add(const Name: String): TTag;
        function Tag(const Name: String): TTag;
        function GetTag(const Name: String): String;
        function SetTag(const Name: String; const Text: String): Integer;
        function SetList(const Name: String; List: TStrings): Integer;
        function Delete(Index: Integer): Boolean; overload;
        function Delete(const Name: String): Boolean; overload;
        function Remove(Index: Integer): Boolean;
        procedure DeleteAllTags;
        procedure Clear; overload;
        function Clear(const Name: String): Integer; overload;
        function Count: Integer;
        function Exists(const Name: String): Integer; overload;
        function TypeCount(const Name: String): Integer;
        function ItemAlreadyOnList(List: TStrings; Item: String): Boolean;
        function GetMultipleList(const Name: String; List: TStrings): Boolean;
        ...
        function AddCoverArt(Name: String; _FileName: String): TCoverArt; overload;
        ...
        function SaveFlacTag(FileName: String; Buffered: Boolean = True;  Stream: TStream = nil): Integer;
        function SaveID3v1Tag(FileName: String; Stream: TStream = nil): Integer;
        function SaveID3v2Tag(FileName: String; Buffered: Boolean = True;  Stream: TStream = nil): Integer;
        function SaveMP4Tag(FileName: String; Buffered: Boolean = True;  Stream: TStream = nil): Integer;
        ...
        function LoadAPEv2CoverArts(FromAPEv2Tag: TAPEv2Tag): Boolean;
        procedure LoadFlacTags;
        function LoadFlacCoverArts(FromFlacTag: TFlacTag): Boolean;
        procedure LoadOggVorbisAndOpusTags;
        function LoadOggVorbisAndOpusCoverArts(FromOggTag: TOpusTag): Boolean;
        procedure LoadID3v1Tags;
        procedure LoadID3v2Tags;
        function LoadID3v2CoverArts(FromID3v2Tag: TID3v2Tag): Boolean;
        procedure LoadMP4Tags;
        function LoadMP4CoverArts(FromMP4Tag: TMP4Tag): Boolean;
        procedure LoadWAVTags;
        {$IFDEF MSWINDOWS}
        procedure LoadWMATags;
        function LoadWMACoverArts(FromWMATag: TWMATag): Boolean;
        {$ENDIF}
    ...
    {$IFDEF MSWINDOWS}
    function WMATagIDToFieldName(TagID: UnicodeString): String;
    function FieldNameToWMATagID(FieldName: String): UnicodeString;
    {$ENDIF}
    ...
    function PictureTypeString2Number(PictureType: String): Integer;
...
var
    ...
    TagsLibraryParseAudioAttributes: Boolean = True;
    TagsLibraryParseCoverArts: Boolean = True;
...
Constructor TTag.Create(_Parent: TTags);
begin
    Inherited Create;
    Self.Parent := _Parent;
end;
...
function TTag.GetIndex: Integer;
begin
    Result := Self.Parent.Tags.IndexOf(Self);
end;
...
constructor TCoverArt.Create(_Parent: TTags);
begin
    inherited Create;
    Self.Parent := _Parent;
...
function TCoverArt.GetIndex: Integer;
begin
    Result := Self.Parent.CoverArts.IndexOf(Self);
end;
...
begin
    Inherited;
    Tags := TList<TTag>.Create;
    CoverArts := TList<TCoverArt>.Create;
    ...
    ParseAudioAttributes := TagsLibraryParseAudioAttributes;
    ParseCoverArts := TagsLibraryParseCoverArts;
end;
...
var
    i: Integer;
begin
    for i := Tags.Count - 1 downto 0 do begin
        Tags[i].Free;
    end;
    Tags.Clear;
    Tags.Capacity := 256;
...
begin
    Result := False;
    if (Index >= CoverArts.Count)
    OR (Index < 0)
    then begin
        Exit;
    end;
    CoverArts[Index].Free;
    CoverArts.Delete(Index);
    Result := True;
end;
...
var
    i: Integer;
begin
    for i := 0 to CoverArts.Count - 1 do begin
        CoverArts[i].Free;
    end;
    CoverArts.Clear;
    CoverArts.Capacity := 128;
end;
...
function TTags.LoadFromFile(_FileName: String): Integer;
var
    Fext: String;
    TagsSize: Integer;
    {$IFDEF MSWINDOWS}
    DetectedAudioFileFormat: TAudioFileFormat;
    {$ENDIF}
begin
    Clear;
    Loaded := False;
    ...
    Fext := UpperCase(ExtractFileExt(_FileName));
    try
        APEv2Tag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    try
        FlacTag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    try
        ID3v1Tag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    try
        ID3v2Tag.ParseAudioAttributes := ParseAudioAttributes;
        ID3v2Tag.ParseCoverArts := ParseCoverArts;
        ID3v2Tag.LoadFromFile(_FileName);
    ...
    try
        MP4Tag.ParseCoverArts := ParseCoverArts;
        MP4Tag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    try
        OggVorbisAndOpusTag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    try
        WAVTag.LoadFromFile(_FileName);
    except
        //* Error
    end;
    {$IFDEF MSWINDOWS}
    //* WMA tag also loads ID3v2 tags and WAVs, so avoid reading it twice
    try
        if (NOT ID3v2Tag.Loaded)
        AND (NOT WAVTag.Loaded)
        then begin
            DetectedAudioFileFormat := DetectAudioFileFormat(_FileName);
            case DetectedAudioFileFormat of
                affAPE, affFlac, affOggFlac, affMPEG, affMP4, affOpus, affVorbis, affTheora, affWAV, affRF64, affAIFF, affAIFC, affDFF, affDSF, affOptimFrog, affMusePack, affWavPack: ; //* Nothing
            else
                WMATag.LoadFromFile(_FileName);
            end;
        end;
    except
        //* Error
    ...
    if ParseWavPackAudioAttributes then begin
        if Fext = '.WV' then begin
            ...
            SourceAudioAttributes.WAVPackAttributes.LoadFromFile(_FileName, APEv2Tag.Size);
    ...
    if ParseMusePackAudioAttributes then begin
        if Fext = '.MPC' then begin
            ...
            SourceAudioAttributes.MusePackAttributes.LoadFromFile(_FileName, ID3v2Tag.Size, TagsSize);
...
function TTags.LoadFromStream(Stream: TStream; Buffered: Boolean = True): Integer;
var
    TagsSize: Integer;
begin
    Clear;
    ...
    try
        Stream.Seek(0, soBeginning);
        FlacTag.LoadFromStream(Stream, Buffered);
    except
        //* Error
    end;
    try
    ...
    try
        Stream.Seek(0, soBeginning);
        ID3v2Tag.LoadFromStream(Stream, Buffered);
        ID3v2Tag.RemoveUnsynchronisationOnAllFrames;
        ID3v2Tag.DeCompressAllFrames;
    except
        //* Error
    ...
    try
        Stream.Seek(0, soBeginning);
        MP4Tag.LoadFromStream(Stream, Buffered);
    except
        //* Error
    end;
    try
    ...
    for i := 0 to APEv2Tag.Count - 1 do begin
        ...
        if APEv2Tag.Frames[i].Format = APEv2Library.ffText then begin
            ...
            then begin
                ...
                TempString := APEv2Tag.Frames[i].GetAsText;
                Delete(Exists('TOTALTRACKS'));
                Add('TOTALTRACKS').Value := Copy(TempString, Pos('/', TempString) + 1, Length(TempString));
                Continue;
        ...
        if ParseCoverArts then begin
            if APEv2Tag.Frames[i].IsCoverArt then begin
                with AddCoverArt(APEv2Tag.Frames[i].Name) do begin
                    ...
                    CoverType := PictureTypeString2Number(APEv2Tag.Frames[i].Name);
                end;
            end;
        end;
    end;
...
function TTags.LoadAPEv2CoverArts(FromAPEv2Tag: TAPEv2Tag): Boolean;
var
    i: Integer;
    APEv2PictureFormat: TAPEv2PictureFormat;
begin
    Result := False;
    for i := 0 to FromAPEv2Tag.Count - 1 do begin
        if FromAPEv2Tag.Frames[i].IsCoverArt then begin
            with AddCoverArt(FromAPEv2Tag.Frames[i].Name) do begin
                FromAPEv2Tag.GetCoverArtFromFrame(i, Stream, APEv2PictureFormat, Description);
                case APEv2PictureFormat of
                    APEv2Library.pfUnknown: begin
                        MIMEType := '';
                        PictureFormat := tpfUnknown;
                ...
                CoverType := PictureTypeString2Number(FromAPEv2Tag.Frames[i].Name);
                Result := True;
            end;
        end;
    end;
    ...
    for i := 0 to FlacTag.Count - 1 do begin
        ...
        if FlacTag.Tags[i].Format = FlacTagLibrary.vcfText then begin
            ...
            then begin
                ...
                TempString := FlacTag.Tags[i].GetAsText;
                Delete(Exists('TOTALTRACKS'));
                Add('TOTALTRACKS').Value := Copy(TempString, Pos('/', TempString) + 1, Length(TempString));
                Continue;
...
function TTags.LoadFlacCoverArts(FromFlacTag: TFlacTag): Boolean;
var
    i: Integer;
    CoverArtInfo: TFlacTagCoverArtInfo;
begin
    Result := False;
    for i := 0 to FromFlacTag.CoverArtCount - 1 do begin
        with AddCoverArt('Cover art ' + IntToStr(i + 1)) do begin
            FromFlacTag.GetCoverArt(i, Stream, CoverArtInfo);
            CoverType := CoverArtInfo.PictureType;
            MIMEType := CoverArtInfo.MIMEType;
            Description := CoverArtInfo.Description;
            Width := CoverArtInfo.Width;
    ...
    for i := 0 to OggVorbisAndOpusTag.Count - 1 do begin
        ...
        if OggVorbisAndOpusTag.Frames[i].Format = OggVorbisAndOpusTagLibrary.otffText then begin
            ...
            then begin
                ...
                TempString := OggVorbisAndOpusTag.Frames[i].GetAsText;
                Delete(Exists('TOTALTRACKS'));
                Add('TOTALTRACKS').Value := Copy(TempString, Pos('/', TempString) + 1, Length(TempString));
                Continue;
...
function TTags.LoadOggVorbisAndOpusCoverArts(FromOggTag: TOpusTag): Boolean;
var
    i: Integer;
    CoverArtInfo: TOpusVorbisCoverArtInfo;
begin
    Result := False;
    for i := 0 to FromOggTag.Count - 1 do begin
        if FromOggTag.Frames[i].IsCoverArt then begin
            with AddCoverArt(FromOggTag.Frames[i].Name) do begin
                FromOggTag.GetCoverArtFromFrame(i, Stream, CoverArtInfo);
                CoverType := CoverArtInfo.PictureType;
                MIMEType := CoverArtInfo.MIMEType;
                Description := CoverArtInfo.Description;
                Width := CoverArtInfo.Width;
...
begin
    //* Delete any exisitng obsolete frames
    for i := ID3v2Tag.FrameCount - 1 downto 0 do begin
        if IsSameFrameID(ID3v2Tag.Frames[i].ID, 'SESC')
        //OR IsSameFrameID(ID3v2Tag.Frames[i].ID, 'TLEN')
        OR IsSameFrameID(ID3v2Tag.Frames[i].ID, 'SEFC')
        OR IsSameFrameID(ID3v2Tag.Frames[i].ID, 'SEBR')
        then begin
    ...
    try
        for i := 0 to ID3v2Tag.FrameCount - 1 do begin
            ...
            if FieldName = 'GENRE' then begin
                with Add('GENRE') do begin
                    Value := ID3v2DecodeGenre(ID3v2Tag.GetUnicodeText(i));
                end;
                Continue;
            end;
            if FieldName <> '' then begin
...
function TTags.LoadID3v2CoverArts(FromID3v2Tag: TID3v2Tag): Boolean;
var
    i: Integer;
...
begin
    Result := False;
    for i := 0 to FromID3v2Tag.FrameCount - 1 do begin
        if IsSameFrameID(FromID3v2Tag.Frames[i].ID, 'APIC') then begin
            with AddCoverArt('Cover art ' + IntToStr(CoverArtCount + 1)) do begin
                FromID3v2Tag.GetUnicodeCoverPictureStream(i, Stream, CoverArtMIMEType, CoverArtDescription, CoverArtType);
                CoverType := CoverArtType;
                MIMEType := CoverArtMIMEType;
                Description := CoverArtDescription;
                PictureFormat := PictureStreamType(Stream);
                Result := True;
            end;
        end;
...
begin
    ...
    //* Xtra
    if MP4Tag.Xtras.Count > 0 then begin
        Text := MP4Tag.GetTextXtra('WM/SharedUserRating');
        if Text <> '' then begin
            Add('SharedUserRating').Value := IntToStr(Trunc((StrToIntDef(Text, 0) / 99) * High(Byte)));
        end;
        Text := MP4Tag.GetTextXtra('WM/AuthorURL');
        if Text <> '' then begin
            Add('AuthorURL').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/PromotionURL');
        if Text <> '' then begin
            Add('PromotionURL').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/SubTitle');
        if Text <> '' then begin
            Add('SubTitle').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Category');
        if Text <> '' then begin
            Add('Category').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Producer');
        if Text <> '' then begin
            Add('Producer').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/EncodedBy');
        if Text <> '' then begin
            Add('EncodedBy').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Conductor');
        if Text <> '' then begin
            Add('Conductor').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/ParentalRating');
        if Text <> '' then begin
            Add('ParentalRating').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Writer');
        if Text <> '' then begin
            Add('Writer').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Director');
        if Text <> '' then begin
            Add('Director').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Publisher');
        if Text <> '' then begin
            Add('Publisher').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Mood');
        if Text <> '' then begin
            Add('Mood').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/ContentDistributor');
        if Text <> '' then begin
            Add('ContentDistributor').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/InitialKey');
        if Text <> '' then begin
            Add('InitialKey').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/Period');
        if Text <> '' then begin
            Add('Period').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/PartOfSet');
        if Text <> '' then begin
            Add('PartOfSet').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/MediaClassPrimaryID');
        if Text <> '' then begin
            Add('MediaClassPrimaryID').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/MediaClassSecondaryID');
        if Text <> '' then begin
            Add('MediaClassSecondaryID').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/WMContentID');
        if Text <> '' then begin
            Add('WMContentID').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/WMCollectionID');
        if Text <> '' then begin
            Add('WMCollectionID').Value := Text;
        end;
        Text := MP4Tag.GetTextXtra('WM/WMCollectionGroupID');
        if Text <> '' then begin
            Add('WMCollectionGroupID').Value := Text;
        end;
    end;
    Self.Loaded := True;
end;

function TTags.LoadMP4CoverArts(FromMP4Tag: TMP4Tag): Boolean;
var
    i, k: Integer;
begin
    Result := False;
    for i := 0 to FromMP4Tag.Count - 1 do begin
        if IsSameAtomName(FromMP4Tag.Atoms[i].ID, 'covr') then begin
            if ParseCoverArts then begin
                for k := 0 to Length(FromMP4Tag.Atoms[i].Datas) - 1 do begin
                    with AddCoverArt('Cover art ' + IntToStr(k + 1)) do begin
                        //* Position should be at start, just to be sure
                        FromMP4Tag.Atoms[i].Datas[k].Data.Seek(0, soBeginning);
                        //* Datas[0] means the first cover stream
                        Stream.CopyFrom(FromMP4Tag.Atoms[i].Datas[k].Data, 0);
                        PictureFormat := PictureStreamType(Stream);
                        if PictureFormat = tpfJPEG then begin
                            MIMEType := 'image/jpeg';
                        end;
...
begin
    ...
    for i := 0 to WAVTag.Count - 1 do begin
        ...
        if FieldName <> '' then begin
            ...
            then begin
                ...
                TempString := WAVTag.Frames[i].GetAsText;
                Delete(Exists('TOTALTRACKS'));
                Add('TOTALTRACKS').Value := Copy(TempString, Pos('/', TempString) + 1, Length(TempString));
                Continue;
    ...
    if WAVTag.BEXT.Loaded then begin
        ...
        Add(BEXT_LoudnessValue).Value := IntToStr(WAVTag.BEXT.LoudnessValue);
        Add(BEXT_LoudnessRange).Value := IntToStr(WAVTag.BEXT.LoudnessRange);
        Add(BEXT_MaxTruePeakLevel).Value := IntToStr(WAVTag.BEXT.MaxTruePeakLevel);
        Add(BEXT_MaxMomentaryLoudness).Value := IntToStr(WAVTag.BEXT.MaxMomentaryLoudness);
        Add(BEXT_MaxShortTermLoudness).Value := IntToStr(WAVTag.BEXT.MaxShortTermLoudness);
...
var
    ...
    List: TStrings;
    {$IFDEF FPC}
    StrWideDescription: UnicodeString;
    StrWideMIMEType: UnicodeString;
    {$ENDIF}
    TempString: String;
begin
    //* Delete any exisitng same tag (so tag type priority decides which tags are actually loaded)
        ...
        then begin
            ...
            if FieldName <> '' then begin
                ...
                then begin
                    ...
                    TempString := WMATag.Frames[i].GetAsText;
                    Delete(Exists('TOTALTRACKS'));
                    Add('TOTALTRACKS').Value := Copy(TempString, Pos('/', TempString) + 1, Length(TempString));
                    Continue;
            ...
            if ParseCoverArts then begin
                if WMATag.Frames[i].Name = g_wszWMPicture then begin
                    with AddCoverArt('Cover art ' + IntToStr(CoverArtCount + 1)) do begin
                        {$IFDEF FPC}
                        WMATag.GetCoverArtFromFrame(i, Stream, StrWideMIMEType, CoverArtType , StrWideDescription);
                        Description := StrWideDescription;
                        MIMEType := StrWideMIMEType;
                        {$ELSE}
...
function TTags.LoadWMACoverArts(FromWMATag: TWMATag): Boolean;
var
    i: Integer;
    //FieldName: String;
    CoverArtType: Byte;
...
begin
    Result := False;
    for i := 0 to FromWMATag.Count - 1 do begin
        if FromWMATag.Frames[i].Name = g_wszWMPicture then begin
            with AddCoverArt('Cover art ' + IntToStr(CoverArtCount + 1)) do begin
                {$IFDEF FPC}
                FromWMATag.GetCoverArtFromFrame(i, Stream, StrWideMIMEType, CoverArtType , StrWideDescription);
                Description := StrWideDescription;
                MIMEType := StrWideMIMEType;
                {$ELSE}
                FromWMATag.GetCoverArtFromFrame(i, Stream, MIMEType, CoverArtType , Description);
                {$ENDIF}
                CoverType := CoverArtType;
                PictureFormat := PictureStreamType(Stream);
                Result := True;
...
function TTags.Add(const Name: String): TTag;
begin
    Result := nil;
    try
        if Tags.Count = Tags.Capacity then begin
            Tags.Capacity := Tags.Capacity + 256;
        end;
...
begin
    ...
    if SameText(FieldName, BEXT_Description) then begin
        ...
        //Result := True;
    end else if SameText(FieldName, BEXT_LoudnessValue) then begin
        WAVTag.BEXT.LoudnessValue := 0;
        //Result := True;
    end else if SameText(FieldName, BEXT_LoudnessRange) then begin
        WAVTag.BEXT.LoudnessRange := 0;
        //Result := True;
    end else if SameText(FieldName, BEXT_MaxTruePeakLevel) then begin
        WAVTag.BEXT.MaxTruePeakLevel := 0;
        //Result := True;
    end else if SameText(FieldName, BEXT_MaxMomentaryLoudness) then begin
        WAVTag.BEXT.MaxMomentaryLoudness := 0;
        //Result := True;
    end else if SameText(FieldName, BEXT_MaxShortTermLoudness) then begin
        WAVTag.BEXT.MaxShortTermLoudness := 0;
        //Result := True;
...
function TTags.Delete(const Name: String): Boolean;
var
    Tag: TTag;
begin
    Result := False;
...
function TTags.Exists(const Name: String): Integer;
var
    i: Integer;
begin
    Result := -1;
...
function TTags.Tag(const Name: String): TTag;
var
    Index: Integer;
begin
    Result := nil;
...
function TTags.TypeCount(const Name: String): Integer;
var
    i: Integer;
begin
    Result := 0;
...
function TTags.Clear(const Name: String): Integer;
var
    i: Integer;
begin
    Result := 0;
...
function TTags.SaveToFile(_FileName: String; TagType: TTagType = ttAutomatic): Integer;
var
    Error: Integer;
    Saved: Boolean;
...
begin
    ...
    FileAge(_FileName, FileDateTime);
    ...
    if TagType = ttAutomatic then begin
        if SameText(_FileName, Self.FileName) then begin
            if APEv2Tag.Loaded then begin
                Error := SaveAPEv2Tag(_FileName);
            ...
            if OggVorbisAndOpusTag.Loaded then begin
                Error := SaveOggVorbisAndOpusTag(_FileName);
        ...
        if NOT Saved then begin
            AudioFileFormat := DetectAudioFileFormat(_FileName);
            case AudioFileFormat of
                ...
                affMPEG, affWAV, affRF64, affAIFF, affAIFC, affDFF ,affDSF: begin
                    ...
                    ID3v2Tag.DeleteFrame('SEFC');
                    ID3v2Tag.DeleteFrame('SESC');
                    ID3v2Tag.DeleteFrame('TLEN');
        ...
        if NOT Saved then begin
            SaveTagTypes := TagTypeFromFileName(_FileName);
    ...
    if Self.PreserveFileDate then begin
        {$IFDEF MSWINDOWS}
        SetDateToFile(_FileName, FileDateTime);
        {$ELSE}
        FileSetDate(_FileName, DateTimeToFileDate(FileDateTime));
        {$ENDIF}
    end;
...
function TTags.SaveToStream(Stream: TStream; Buffered: Boolean = True; TagType: TTagType = ttAutomatic): Integer;
var
    Error: Integer;
    AudioFileFormat: TAudioFileFormat;
begin
    ...
    if TagType = ttAutomatic then begin
        ...
        case AudioFileFormat of
            ...
            affFlac, affOggFlac: begin
                Error := SaveFlacTag('', Buffered, Stream);
                Result := FlacTagLibraryErrorCodeToTagsLibraryErrorCode(Error);
            end;
            affMPEG, affWAV, affRF64, affAIFF, affAIFC, affDFF, affDSF: begin
                Error := SaveID3v2Tag('', Buffered, Stream);
                Result := ID3v2TagLibraryErrorCodeToTagsLibraryErrorCode(Error);
                if (AudioFileFormat = affWAV)
                OR (AudioFileFormat = affRF64)
                then begin
            ...
            affMP4: begin
                Error := SaveMP4Tag('', Buffered, Stream);
                Result := MP4TagLibraryErrorCodeToTagsLibraryErrorCode(Error);
            end;
...
function TTags.SaveFlacTag(FileName: String; Buffered: Boolean = True; Stream: TStream = nil): Integer;
var
    i, k: Integer;
...
begin
    ...
    if Assigned(Stream) then begin
        Stream.Seek(0, soBeginning);
        Result := FlacTag.SaveToStream(Stream, Buffered);
    end else begin
        Result := FlacTag.SaveToFile(FileName, Buffered);
    end;
end;
...
function TTags.SaveID3v2Tag(FileName: String; Buffered: Boolean = True; Stream: TStream = nil): Integer;
var
    i: Integer;
    FrameName: String;
    FrameID: TFrameID;
...
begin
    ...
    if Assigned(Stream) then begin
        Stream.Seek(0, soBeginning);
        Result := ID3v2Tag.SaveToStream(Stream, Buffered);
    end else begin
        Result := ID3v2Tag.SaveToFile(FileName, Buffered);
    end;
end;

function TTags.SaveMP4Tag(FileName: String; Buffered: Boolean = True; Stream: TStream = nil): Integer;
var
    i: Integer;
    AtomName: String;
    Atom: TMP4Atom;
...
begin
    ...
    for i := 0 to Count - 1 do begin
        ...
        if AtomName = '' then begin
            ...
            if AtomName = 'pgap' then begin
                ...
                MP4Tag.SetInteger16('tmpo', StrToIntDef(Tags[i].Value, 0));
                ...
                if TypeCount(Tags[i].Name) > 1 then begin
                    ...
                    try
                        GetMultipleList(Tags[i].Name, MultipleValues);
                        MP4Tag.SetMultipleValuesMultipleAtoms(AtomName, MultipleValues);
                    finally
                        FreeAndNil(MultipleValues);
                    end;
                end else begin
    ...
    if Assigned(Stream) then begin
        Stream.Seek(0, soBeginning);
        Result := MP4Tag.SaveToStream(Stream, Buffered, MP4KeepPadding);
    end else begin
...
begin
    ...
    for i := 0 to Count - 1 do begin
        ...
        end else if SameText(Tags[i].Name, BEXT_LoudnessValue) then begin
            WAVTag.BEXT.LoudnessValue := StrToIntDef(Tags[i].Value, 0);
        end else if SameText(Tags[i].Name, BEXT_LoudnessRange) then begin
            WAVTag.BEXT.LoudnessRange := StrToIntDef(Tags[i].Value, 0);
        end else if SameText(Tags[i].Name, BEXT_MaxTruePeakLevel) then begin
            WAVTag.BEXT.MaxTruePeakLevel := StrToIntDef(Tags[i].Value, 0);
        end else if SameText(Tags[i].Name, BEXT_MaxMomentaryLoudness) then begin
            WAVTag.BEXT.MaxMomentaryLoudness := StrToIntDef(Tags[i].Value, 0);
        end else if SameText(Tags[i].Name, BEXT_MaxShortTermLoudness) then begin
            WAVTag.BEXT.MaxShortTermLoudness := StrToIntDef(Tags[i].Value, 0);
        //* CART
            ...
            CART_TIMER.Value := StrToUIntDef(Copy(Tags[i].Value, Pos(' ', Tags[i].Value) + 1, Length(Tags[i].Value)), 0);
...
function TTags.GetMultipleList(const Name: String; List: TStrings): Boolean;
var
    i: Integer;
begin
    Result := False;
...
begin
    ...
    if MP4Tag.Loaded then begin
        Result := Result + MP4Tag.CalculateSize;
    end;
    if OggVorbisAndOpusTag.Loaded then begin
...
function TTags.GetTag(const Name: String): String;
var
    Index: Integer;
begin
    Result := '';
...
function TTags.SetTag(const Name: String; const Text: String): Integer;
var
    i: Integer;
    l: Integer;
begin
...
function TTags.SetList(const Name: String; List: TStrings): Integer;
var
    i: Integer;
    l: Integer;
begin
...
procedure TTags.ReSetAlreadyParsedState;
var
    i: Integer;
begin
    for i := 0 to Tags.Count - 1 do begin
        Tags[i].AlreadyParsed := False;
    end;
    for i := 0 to CoverArts.Count - 1 do begin
        CoverArts[i].AlreadyParsed := False;
    end;
end;
...
begin
    Result := nil;
    try
        Result := TCoverArt.Create(Self);
        Result.Name := Name;
        Result.Description := 'No description';
        Result.PictureFormat := tpfUnknown;
        CoverArts.Add(Result);
    except
        //*
    end;
end;
...
function TTags.AddCoverArt(Name, _FileName: String): TCoverArt;
var
    PictureStream: TFileStream;
    PictureMagic: Word;
    PictureMagicTIFF: Cardinal;
    MIMEType: String;
begin
    Result := nil;
    if FileExists(_FileName) then begin
        try
            PictureStream := TFileStream.Create(_FileName, fmOpenRead);
            try
                ...
                if PictureMagic = MAGIC_JPG then begin
                ...
                end else if PictureMagic = MAGIC_PNG then begin
                    MIMEType := 'image/png';
                    {
                ...
                end else if PictureMagic = MAGIC_GIF then begin
                    MIMEType := 'image/gif';
                    {
                ...
                end else if PictureMagic = MAGIC_BMP then begin
                    MIMEType := 'image/bmp';
                    {
                    ...
                    PictureStream.Seek(0, soBeginning);
                    PictureStream.Read(PictureMagicTIFF, 4);
                    PictureStream.Seek(0, soBeginning);
                    if (PictureMagicTIFF = MAGIC_TIFF_LE)
                    OR (PictureMagicTIFF = MAGIC_TIFF_BE)
                    then begin
                        MIMEType := 'image/tiff';
                    end;
                end;
    ...
    if Assigned(Result) then begin
        Result.Description := ExtractFileName(_FileName);
    end;
end;
...
begin
    Result := TagID;
    //* Skip WMA specific non-tag data
    if (TagID = g_wszWMDuration)
    OR (TagID = g_wszWMBitrate)
    OR (TagID = g_wszWMSeekable)
    OR (TagID = g_wszWMStridable)
    OR (TagID = g_wszWMBroadcast)
    OR (TagID = g_wszWMProtected)
    OR (TagID = g_wszWMTrusted)
    OR (TagID = g_wszWMSignature_Name)
    OR (TagID = g_wszWMHasAudio)
    OR (TagID = g_wszWMHasImage)
    OR (TagID = g_wszWMHasScript)
    OR (TagID = g_wszWMHasVideo)
    OR (TagID = g_wszWMCurrentBitrate)
    OR (TagID = g_wszWMOptimalBitrate)
    OR (TagID = g_wszWMHasAttachedImages)
    OR (TagID = g_wszWMSkipBackward)
    OR (TagID = g_wszWMSkipForward)
    OR (TagID = g_wszWMNumberOfFrames)
    OR (TagID = g_wszWMFileSize)
    OR (TagID = g_wszWMHasArbitraryDataStream)
    OR (TagID = g_wszWMHasFileTransferStream)
    OR (TagID = g_wszWMContainerFormat)
    OR (TagID = 'WMFSDKVersion')
    OR (TagID = 'WMFSDKNeeded')
    OR (TagID = 'IsVBR')
    OR (TagID = g_wszWMASFPacketCount)
    OR (TagID = g_wszWMASFSecurityObjectsSize)
    OR (TagID = g_wszWMPicture)
    OR (TagID = g_wszWMStreamTypeInfo)
    OR (TagID = g_wszWMPeakBitrate)
    then begin
        Result := '';
        Exit;
    end;
    ...
    if TagID = g_wszWMTrackNumber then begin
        Result := 'TRACKNUMBER';
        Exit;
    end;
...
constructor TSourceAudioAttributes.Create(_Parent: TTags);
begin
    inherited Create;
    Self.Parent := _Parent;
end;
...
begin
    ...
    for i := Low(Parent.TagLoadPriority) to High(Parent.TagLoadPriority) do begin
...
begin
    ...
    for i := Low(Parent.TagLoadPriority) to High(Parent.TagLoadPriority) do begin
        ...
        if Parent.TagLoadPriority[i] = TagsLibrary.ttMP4 then begin
            if Parent.MP4Tag.AudioResolution <> 0 then begin
                Result := Parent.MP4Tag.AudioResolution;
                Exit;
            end;
        end;
        {$IFDEF MSWINDOWS}
...
begin
    ...
    for i := Low(Parent.TagLoadPriority) to High(Parent.TagLoadPriority) do begin
        ...
        if Parent.TagLoadPriority[i] = TagsLibrary.ttMP4 then begin
            if Parent.MP4Tag.AudioChannelCount <> 0 then begin
                Result := Parent.MP4Tag.AudioChannelCount;
                Exit;
            end;
        end;
        {$IFDEF MSWINDOWS}
...
begin
    ...
    if Parent.FlacTag.Valid then begin
        if Parent.FlacTag.PlayTime <> 0 then begin
                ...
                Exit;
            {
                ...
                if Parent.OggVorbisAndOpusTag.Info.PlayTime <> 0 then begin
                    if Parent.OggVorbisAndOpusTag.Info.OpusParameters.SampleRate <> 0 then begin
                        Result := Round(Parent.OggVorbisAndOpusTag.Info.PlayTime * Parent.OggVorbisAndOpusTag.Info.OpusParameters.SampleRate);
                        Exit;
                    end else if Parent.OggVorbisAndOpusTag.Info.VorbisParameters.SampleRate <> 0 then begin
                        Result := Round(Parent.OggVorbisAndOpusTag.Info.PlayTime * Parent.OggVorbisAndOpusTag.Info.VorbisParameters.SampleRate);
                        Exit;
                    end;
                end;
                }
...
begin
    ...
    for i := Low(Parent.TagLoadPriority) to High(Parent.TagLoadPriority) do begin
        ...
        if Parent.TagLoadPriority[i] = TagsLibrary.ttMP4 then begin
            if Parent.MP4Tag.AudioSampleRate <> 0 then begin
                Result := Parent.MP4Tag.AudioSampleRate;
                Exit;
            end;
        end;
        {$IFDEF MSWINDOWS}
    ...
    try
        ...
        try
            ...
            case Magic of
                ...
                PictureStream.Seek(0, soBeginning);
                PictureStream.Read(MagicTIFF, SizeOf(MagicTIFF));
                if (MagicTIFF = MAGIC_TIFF_LE)
                OR (MagicTIFF = MAGIC_TIFF_BE)
                then begin
                    Result := tpfTIFF;
                end;
            end;
...
begin
    ...
    try
        ...
        MP4AtomSize := MP4TagLibrary.ReverseBytes32(MP4AtomSize);
...
begin
    Result := $00;
    if SameText(PictureType, 'Other') then begin
        Result := $00;
        Exit;
    end;
    if SameText(PictureType, '32x32 pixels ''file icon'' (PNG only)') then begin
        Result := $01;
        Exit;
    end;
    if SameText(PictureType, 'Other file icon') then begin
        Result := $02;
        Exit;
    end;
    if SameText(PictureType, 'Cover (front)') then begin
        Result := $03;
        Exit;
    end;
    if SameText(PictureType, 'Cover (back)') then begin
        Result := $04;
        Exit;
    end;
    if SameText(PictureType, 'Leaflet page') then begin
        Result := $05;
        Exit;
    end;
    if SameText(PictureType, 'Media (e.g. label side of CD)') then begin
        Result := $06;
        Exit;
    end;
    if SameText(PictureType, 'Lead artist/lead performer/soloist') then begin
        Result := $07;
        Exit;
    end;
    if SameText(PictureType, 'Artist/performer') then begin
        Result := $08;
        Exit;
    end;
    if SameText(PictureType, 'Conductor') then begin
        Result := $09;
        Exit;
    end;
    if SameText(PictureType, 'Band/Orchestra') then begin
        Result := $0A;
        Exit;
    end;
    if SameText(PictureType, 'Composer') then begin
        Result := $0B;
    end;
    if SameText(PictureType, 'Lyricist/text writer') then begin
        Result := $0C;
        Exit;
    end;
    if SameText(PictureType, 'Recording Location') then begin
        Result := $0D;
        Exit;
    end;
    if SameText(PictureType, 'During recording') then begin
        Result := $0E;
        Exit;
    end;
    if SameText(PictureType, 'During performance') then begin
        Result := $0F;
        Exit;
    end;
    if SameText(PictureType, 'Movie/video screen capture') then begin
        Result := $10;
        Exit;
    end;
    if SameText(PictureType, 'A bright coloured fish') then begin
        Result := $11;
        Exit;
    end;
    if SameText(PictureType, 'Illustration') then begin
        Result := $12;
        Exit;
    end;
    if SameText(PictureType, 'Band/artist logotype') then begin
        Result := $13;
        Exit;
    end;
    if SameText(PictureType, 'Publisher/Studio logotype') then begin
        Result := $14;
        Exit;
    end;
end;
...
Initialization
    ...
    TagsLibraryDefaultTagLoadPriority[3] := ttWAV;
    TagsLibraryDefaultTagLoadPriority[4] := ttID3v2;
    TagsLibraryDefaultTagLoadPriority[5] := ttAPEv2;
    TagsLibraryDefaultTagLoadPriority[6] := ttID3v1;
    TagsLibraryDefaultTagLoadPriority[7] := ttWMA;
